			...
			[
			 	{
					...
					[
						{"icon":"icon-sys","menuid":"101","menuname":"会议类型","url":"mettype.html","menus": [],"pid": 100}	,
						{"icon":"icon-sys","menuid":"102","menuname":"会议地点","url":"place.html","menus": [],"pid": 100}
					],
                    "pid": 0,
                    "url": ""
			 	},
                {
                    "icon":"icon-sys","menuid":"200","menuname":"人事管理","menus":
                        [
                            {"icon":"icon-sys","menuid":"201","menuname":"部门管理","url":"dep.html","menus": [],"pid": 200}	,
                            {"icon":"icon-sys","menuid":"202","menuname":"员工管理","url":"emp.html","menus": [],"pid": 200}
                        ],
                    "pid": 0,
                    "url": ""
                }
...
$(function(){	
	...
	$.ajax({
		...
		success: function(rtn){
			//给菜单赋值
            // var dataobj = eval("("+rtn+")");
            // var item=JSON.stringify(rtn);
            // console.log(item);
            // alert(item);
			_menus=rtn;
			InitLeftMenu();
	...
	$('#loginOut').click(function(){
        // sessionStorage.removeItem("login_emp");
        // location.href("login");
		$.ajax({
			url: 'login_loginOut',
			method:'get',
			success: function(rtn){
				// $.messager('确定',rtn.message,'info');
                location.href= "login";
			}
		});
	});
...
function serverLogin() {
    ...
    if (rePwd == '') {
        ...
        $.messager.alert('提示','请在一次输入密码！','info');
    }

    if (newPwd!= rePwd) {
        // msgShow('系统提示', '两次密码不一至！请重新输入', 'warning');
        $.messager.alert('提示','两次密码不一至！请重新输入！','info');
    }
    if (newPwd == rePwd) {
        $.ajax({
            url: 'emp_updatePwd',
